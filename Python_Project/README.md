...
**"Can't open lib 'ODBC Driver 17 for SQL Server'" or "Data source name not found"**
The ODBC driver is not installed, or the name in `.env` does not match.
Install it from the link in Requirements, then check the exact name in
"ODBC Data Sources (64-bit)" → Drivers tab and copy it into `DB_DRIVER`.

**"Could not reach SQL Server"**
Check the SQL Server service is running: press Start, type `services.msc`,
find **SQL Server (SQLEXPRESS)** or **SQL Server (MSSQLSERVER)** and make sure
it says Running. Also double-check `DB_SERVER` in `.env`.

**"Cannot open database"**
You have not run `database/schema.sql` yet. Do Step 5.

**"Invalid object name 'ClassesRegistration'"**
The database exists but the tables do not. Run `database/schema.sql` again.

**"Login failed for user"**
If you are using SQL Server Authentication, check `DB_USER` and `DB_PASSWORD`.
If you are using Windows Authentication, make sure `DB_TRUSTED_CONNECTION=yes`.

**`python` is not recognised**
Python was installed without "Add to PATH". Re-run the Python installer, choose
Modify, and tick that option.

**`pip install` fails on pyodbc**
Make sure your virtual environment is activated (you should see `(venv)`), and
that you are on 64-bit Python. Then try again.

**The analysis table is all zeros**
No feedback has been submitted for that class yet. Use the Rating button on the
search results to add some.

**The registration page shows a broken image**
`images/register_img.png` is referenced by the original `registration.aspx` but
that file was never in the project, so the image was broken in the C# version
too. Drop any picture into `static/images/` with that name to fix it.

**Port 5000 is already in use**
Change the last line of `main.py` to a different port, for example
...
1. Create a new **empty** repository on <https://github.com> (do not add a
...
git init
git add .
git commit -m "Python version of project"
git branch -M main
...
| `git add .` | Stages every file, except the ones listed in `.gitignore`. |
| `git commit -m "..."` | Saves a snapshot of the staged files, with a message. |
| `git branch -M main` | Names the branch `main`, which is what GitHub expects. |
| `git remote add origin URL` | Tells Git where your GitHub repository is. |
| `git push -u origin main` | Uploads your commit to GitHub. |

Before pushing, run `git status` and confirm that `.env` and `venv/` are **not**
in the list. They should be ignored automatically.
